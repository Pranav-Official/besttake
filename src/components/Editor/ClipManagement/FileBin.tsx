...
import { SourceFile, ProjectMetadata } from "../../../types/constants";
      ...
      <div className="p-4 border-b border-[#1d417c] flex items-center justify-between shrink-0">
        <div className="flex gap-6">
          <button
            ...
            className={`text-xs font-bold uppercase tracking-wider transition-colors ${activeTab === "media" ? "text-[#9cb2d7] border-b-2 border-[#9cb2d7]" : "text-[#9cb2d7]/40 hover:text-[#9cb2d7]/70"}`}
          >
            Media Bin
          </button>
          <button
            onClick={() => setActiveTab("projects")}
            className={`text-xs font-bold uppercase tracking-wider transition-colors ${activeTab === "projects" ? "text-[#9cb2d7] border-b-2 border-[#9cb2d7]" : "text-[#9cb2d7]/40 hover:text-[#9cb2d7]/70"}`}
          >
            Projects
          </button>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {projectsList.map((project: ProjectMetadata) => (
              <div
                ...
                className={`relative aspect-video rounded-lg border-2 overflow-hidden cursor-pointer transition-all group flex flex-col items-center justify-center p-4 gap-2 ${
                  activeProject?.id === project.id
                    ? "border-[#9cb2d7] bg-[#1d417c]/30"
                    : "border-[#1d417c] bg-[#011626]/40 hover:border-[#1d417c]/80"
                }`}
              >
                <div className="w-8 h-8 bg-[#1d417c]/50 rounded flex items-center justify-center text-[#9cb2d7]">
                  <svg
                    className="w-4 h-4"
                    fill="none"
                    stroke="currentColor"
                ...
                <p className="text-[10px] font-bold text-white text-center line-clamp-2 px-2">
                  {project.name}
                </p>
                <p className="text-[8px] text-[#9cb2d7]">
